/*
 * Click nbfs://nbhost/SystemFileSystem/Templates/Licenses/license-default.txt to change this license
 * Click nbfs://nbhost/SystemFileSystem/Templates/Classes/Class.java to edit this template
 */
package arreglos;

import java.util.Scanner;

/**
 *
 * @author Lee Brandon
 */
public class Ejercicio13_2 {
    /*
    Leer 10 enteros en una tabla. Guardar en otra tabla los elementos pares de la primera y a continuación los elementos impares. (3 TABLAS DIFERENTES)
    */
    public static void main(String[] args) {
        Scanner leer = new Scanner(System.in);
        //Creamos array 
        int numeros[] = new int[10], contP=0, contI=0;
        
        //Llenar Array
        System.out.println("=== PARES O IMPARES ===");
        for (int i = 0; i < numeros.length; i++) {
            System.out.print((i+1)+".Numero: ");
            numeros[i] = Integer.parseInt(leer.nextLine());
            if (numeros[i]%2==0) {
                contP++;
            }else {
                contI++;
            }
        }
        //Creamos tablas pares e impares
        int pares[] = new int[contP];
        int[] impares = new int[contI];
        
        contP=0;
        contI=0;
        
        //Llenamos array dep ares e impares
        for (int i = 0; i < numeros.length; i++) {
            if (numeros[i]%2==0) {
                pares[contP] = numeros[i];
                contP++;
            } else {
                impares[contI] = numeros[i];
                contI++;
            }
        }
        
        //Mostramos las tabals
        System.out.println("\n=== RESULTADO ===");
        for (int par : pares) {
            System.out.print(par+" ");
        }
        for (int impare : impares) {
            System.out.print(impare+" ");
        }
        System.out.println("");
    }
}
